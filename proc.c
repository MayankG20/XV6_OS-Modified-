  ...
  for(;;){
    ...
    acquire(&ptable.lock);
      ...
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        ...
        // It should have changed its p->state before coming r.
        c->proc = 0;
      }
        ...
        struct proc *mp;
        struct proc *mn=ptable.proc;
        int mnt=10000;
        for(mp=ptable.proc; mp< &ptable.proc[NPROC];mp++){
          if(mp->stime < mnt && mp->state==RUNNABLE){
            mn=mp;
            mnt=mp->stime;
          }
        }
        ...
        int mnp=10000;
        struct proc * mp1=0;
        for(mp1=ptable.proc; mp1 < &ptable.proc[NPROC];mp1++){
          if(mp1->state!=RUNNABLE)
            continue;
          mnp=mp1->prty;
          mn=mp1;
          for(mp=ptable.proc; mp< &ptable.proc[NPROC];mp++){
            if(mp->prty < mnp && mp->state==RUNNABLE){
              mn=mp;
              mnp=mp->prty;
            }
          }
...
yield(void)
{
  acquire(&ptable.lock);  //DOC: yieldlock
    ...
    for(struct proc *p=ptable.proc;p<&ptable.proc[NPROC];p++){
      if(p->state == RUNNABLE && p->prty <= myproc()->prty){
